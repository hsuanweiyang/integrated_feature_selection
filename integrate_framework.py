...
class IntegrateFeatureSelection:
    ...
    def old_type_one_select_feature(self):
    ...
    def old_type_one_sffs(self, feature_amount):
        selected_feature_set = []
        computation_iterations = 0
        max_acc = -1
        start_time = timeit.default_timer()
        tested_feature = 0
        while int(float(max_acc)) != 100 and tested_feature < feature_amount:
            local_max_acc = -1
            ...
            if local_max_acc > max_acc:
                ...
                selected_feature_set, remove_features, bc_max_acc, bc_computation_iterations \
    ...
    def old_type_one_modified_sffs(self, feature_test_uplimit):
        selected_feature_set = []
        computation_iterations = 0
        max_acc = -1
        start_time = timeit.default_timer()
    ...
    def type_one_select_feature(self):
        tmp_select_feature_set = []
        for each_fs_method in self.feature_index:
            logging.info('method:{0}'.format(each_fs_method))
            if len(self.feature_index[each_fs_method]['original']) > 0:
                tmp_select_feature = self.feature_index[each_fs_method]['original'][0]
            else:
                continue
            if tmp_select_feature in tmp_select_feature_set:
                continue
            else:
                tmp_select_feature_set.append(tmp_select_feature)
        logging.info('feature set:{0}'.format(tmp_select_feature_set))
        return tmp_select_feature_set
    ...
    def type_one_sffs(self, feature_amount):
        ...
        while int(float(max_acc)) != 100 and tested_feature < feature_amount:
            local_max_acc = -1
            tmp_feature_set = self.type_one_select_feature()
            for tmp_feature in tmp_feature_set:
                tmp_all_feature = selected_feature_set + [tmp_feature]
                tmp_feature_dataset = self.tmp_selected_dataset(tmp_all_feature)
                predict = IntegratedPredictor(self.label, tmp_feature_dataset, self.fold)
                local_acc = predict.select_predictor(self.clf)
                computation_iterations += 1
                logging.debug('Feature Index:{0}\tlocal:{1}'.format(tmp_feature, local_acc))
                if local_acc > local_max_acc:
                    local_max_acc = local_acc
                    tmp_selected_feature = tmp_feature
                    logging.debug('\tLocal Max:{0}'.format(local_max_acc))
            ...
            if local_max_acc > max_acc:
                ...
                bc_selected_feature_set, remove_features, bc_max_acc, bc_computation_iterations \
                    = self.backward_selection(selected_feature_set, max_acc)
                computation_iterations += bc_computation_iterations
                if bc_max_acc > max_acc:
                    max_acc = bc_max_acc
    ...
    def type_two_sffs(self, feature_test_uplimit):
        selected_feature_set = []
        computation_iterations = 0
        tested_feature = 0
        max_acc = -1
        ...
        while int(float(max_acc)) != 100 and tested_feature < feature_test_uplimit:
            tmp_feature = feature_priority_list.pop(0)
...
def cv_work(train_test_group):
    ...
    if train_test_group[7] == '110':
        output_file_name = 'type-one-sfs'
        #hybrid_function = select.
    elif train_test_group[7] == '120':
        output_file_name = 'type-one-sffs'
        hybrid_function = select.type_one_sffs
    elif train_test_group[7] == '121':
        output_file_name = 'type-one-modified-sffs'
        hybrid_function = select.type_one_modified_sffs
    elif train_test_group[7] == '210':
        output_file_name = 'type-two-sfs'
        #hybrid_function =
    elif train_test_group[7] == '220':
        output_file_name = 'type-two-sffs'
        hybrid_function = select.type_two_sffs
    elif train_test_group[7] == '221':
        output_file_name = 'type-two-modified-sffs'
        hybrid_function = select.type_two_modified_sffs
    else:
        print 'Hybrid mode error'
        exit()
    output_file_name += '_{0}-{1}'.format(train_test_group[6], train_test_group[5])
    result_accuracy, selected_feature_set, selected_feature_set_bymethod, iterations, time_required \
        = hybrid_function(len(train_test_group[1][0]))
    ...
    return [output_str, output_file_name]
...
if __name__ == '__main__':
    ...
    multi_process.close()
    multi_process.join()
    output_file = open(cv_reports[0][1], mode='w')
    for each_report in cv_reports:
        output_file.write(each_report[0])
    output_file.close()
    '''
    fold_output = ''
    for each_fold in train_test_fold:
